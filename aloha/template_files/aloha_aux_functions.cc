...
void ixxxxx(double p[4], double fmass, int nhel, int nsf, int flv, ALOHAOBJ &fi)
{
  ...
  {
    if (p[3] < 0.0 and (p[1] != 0.0 or p[2] != 0.0) and (p[0] - p[3]) > 0.0)
    {
      ...
      sqp0p3 = sqrt((p[1] * p[1] + p[2] * p[2])/(p[0] - p[3])) * nsf;
    }
    else if (p[1] == 0.0 and p[2] == 0.0 and p[3] < 0.0)
    {
      sqp0p3 = 0.0;
    }
    else
...
void oxxxxx(double p[4], double fmass, int nhel, int nsf, int flv, ALOHAOBJ &fo)
{
  ...
  else
  {
    ...
    else if((p[1] == 0.00) and (p[2] == 0.00) and (p[3] < 0.00))
    {
      sqp0p3 = 0.00;
    }
    else
...
void txxxxx(double p[4], double tmass, int nhel, int nst, ALOHAOBJ2D &tc)
{
  ...
  for ( int i =0; i<4;i++){
	...
	for( j=0; j<4; j++ )
	{
		for( i=0; i<4; i++ ) tc.W[j*4+i] = ft[j][i];
	}
}
#include <complex>
#include <cmath>
