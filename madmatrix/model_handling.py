...
class MadMatrixALOHAWriter(aloha_writers.ALOHAWriterForGPU):
    ...
    def use_fixp2(self):
        ...
        extra FIXP2 argument. FIXP2 is a per-event quantity (a SIMD fptype_sv):
        for the entries where it is non-zero the routine uses it in place of the
        computed p^2 in the propagator denominator. It is restricted to the
        standard denominator: custom propagators (self.routine.denominator) and
        the P1N numerator variant are excluded.
    ...
    def get_one_momenta_def(self, i, strfile):
        ...
        if self.nodeclare:
            if ptype == 'double_v':
                ...
                # (static_cast each pvec[] from fptype_momenta to fptype_denom: the two may
                # differ, e.g. fptype_momenta=double while fptype_denom=fptype2=float)
                if i == self.outgoing:
                    strfile.write('    const fptype_denom_sv dP%d[4] = { ' % i )
                    for j in range(4):
                        sign = self.get_P_sign(i) if self.get_P_sign(i) else '+'
                        element = 'static_cast<fptype_denom_sv>(%(sign)s%(type)s%(i)d.pvec[%(j)d])' % {'j':j,'type': type, 'i': i, 'sign': sign}
                        strfile.write(element + (', ' if j<3 else ''))
                    strfile.write(' };\n')
            else:
    ...
    def define_expression(self):
        ...
        if not self.offshell:
            ...
            if 'L' not in self.tag:
                ...
                # OM - FP64 (fptype_invmass) numerator for the m_ij/FIXP2 path: same coupling as
                # the normal denominator below but cast to invmass precision. Captured here while
                # pre_coup/post_coup are still the raw (pointer-deref) wrappers, before the
                # cxtype_denom casts are added.
                mydict['coupraw'] = '%s%s%s' % (mydict['pre_coup'], mydict['coup'], mydict['post_coup'])
                if mydict['coup'] != 'one':
                    mydict['invnum'] = 'static_cast<fptype_invmass>(Ccoeff) * static_cast<cxtype_invmass>(%(coupraw)s)' % mydict
                else:
                    mydict['invnum'] = 'static_cast<fptype_invmass>(%(coupraw)s)' % mydict
                ...
                if not aloha.complex_mass:
                    ...
                    if self.routine.denominator:
                        ...
                        # OM - per-event m_ij (FIXP2) handling. FIXP2 is the caller-supplied
                        # invariant mass^2 of this offshell current, gated in by the channelId
                        # (0 where not provided). The denominator is computed in ONE precision
                        # only, chosen per event with a (lazy) ternary so the other path is not
                        # evaluated:
                        #  - FIXP2 == 0 (no m_ij): the NORMAL cxtype_denom propagator, p^2 from
                        #    the momenta in fptype_denom, unchanged from the non-FIXP2 path.
                        #  - FIXP2 != 0 (m_ij provided): denominator computed in fptype_invmass
                        #    (FP64) directly from FIXP2, then down-cast to cxtype_denom_sv.
                    ...
                    # This affects 'denom = COUP' in HelAmps_sm.cc
                    if self.use_fixp2():
                        # OM - complex-mass scheme: same lazy per-event selection as the real-mass
                        # FIXP2 path (FIXP2 == 0 -> compute in fptype_denom; FIXP2 != 0 -> compute
