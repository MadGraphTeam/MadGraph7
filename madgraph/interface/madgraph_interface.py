...
class HelpToCmd(cmd.HelpCmd):
    ...
    def help_polarization(self):
        ...
        logger.info("Polarization at fixed-order NLO:",'$MG:BOLD')
        logger.info(" > A polarized massive particle needs a frame, so it is allowed for the NLO modes")
        logger.info("   that have one: [QCD], [real=QCD] and [LOonly=QCD] take 'me_frame' from the")
        logger.info("   run_card, and [virt=QCD] (or standalone MadLoop) takes the frame from the")
        logger.info("   momenta the caller supplies. A polarized massless particle needs no frame and")
        logger.info("   was always allowed.")
