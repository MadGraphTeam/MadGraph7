...
ifeq ($(FPTYPE),d) # all64
  ...
  # C++/SIMD needs a separate FP64 denom buffer (structural change, deferred to after the
  # backend-separation PR) -> fall back to color32 (m) for now; keep real v only on GPU.
  GPUFLAGS += -DMGONGPU_FPTYPE_FLOAT -DMGONGPU_FPTYPE2_FLOAT -DMGONGPU_FPTYPE_MOMENTA_DOUBLE
ifeq ($(filter $(BACKEND),cuda hip),)
  $(info FPTYPE=v falls back to FPTYPE=m for BACKEND=$(BACKEND) (SIMD denom64 pending))
  CXXFLAGS += -DMGONGPU_FPTYPE_DOUBLE -DMGONGPU_FPTYPE2_FLOAT
else
