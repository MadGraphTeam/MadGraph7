      SUBROUTINE SBORN_HEL(P,ANS_SUMMED,LOC_SAVEAMP,WGT_HEL)
C     
...
C     
      ...
      DOUBLE COMPLEX LOC_SAVEAMP(1,MAX_BHEL)
C     
C     VARIABLES
C     
      INTEGER I, J
...
C     look for orders which match the born order constraint 
      CALL SBORN_HEL_SPLITORDERS(P,ANS,LOC_SAVEAMP,WGT_HEL)
      ANS_SUMMED = 0D0
      DO I = 1, NSQAMPSO
      ...
      SUBROUTINE SBORN_HEL_SPLITORDERS(P1,ANS,LOC_SAVEAMP,WGT_HEL)
C     
...
C     write(*,*) 'Error in sborn_hel_splitorders: momenta not the same'
C     //' in the born'
C     stop
C     endif
C     enddo
C     else
C     write(*,*) 'Error in sborn_hel_splitorders: this should be'
C     //' called only with calculatedborn = true'
C     stop
C     endif
      DO I=0,NSQAMPSO
        ANS(I) = 0D0
      ...
      DO IHEL=1,NCOMB
        IF (GOODHEL(IHEL,NFKSPROCESS)) THEN
          CALL BORN_HEL_SPLITORDERS(P1,IHEL,T,LOC_SAVEAMP)
          DO I=1,NSQAMPSO
      ...
      SUBROUTINE BORN_HEL_SPLITORDERS(P,HELL,ANS,LOC_SAVEAMP)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     write(*,*) 'Error in b_sf: color_linked borns should be called'
C     //' only with calculatedborn = true'
C     stop
C     elseif (calculatedBorn) then
      DO I=1,NGRAPHS
        AMP(I)=LOC_SAVEAMP(I,HELL)
      ENDDO
C     endif
C     JAMPs contributing to orders QCD=0 QED=2
     ...
     $     *DCONJG(JAMP(I,M))
      ...
      SUBROUTINE PICKHELICITYMC(P,GOODHEL,HEL,IHEL_OUT,VOL,LOC_SAVEAMP)
      IMPLICIT NONE
      INCLUDE 'nexternal.inc'
      INCLUDE 'orders.inc'
      INCLUDE 'born_nhel.inc'
      ...
      CALL SBORN_HEL(P,BORN_WGT_RECOMP_DIRECT,LOC_SAVEAMP,WGT_HEL)
