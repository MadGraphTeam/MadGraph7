...
C     
      ...
      CALL SMATRIX5_AMP(P,DUMMY_AMP_SPLIT,ANS_SUMMED)
      ...
      SUBROUTINE SMATRIX5_AMP(P,RET_AMP_SPLIT,ANS_SUMMED)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     
      INTEGER I,J
      REAL*8 ANS(0:NSQAMPSO)
      LOGICAL KEEP_ORDER5(NSQAMPSO), FIRSTTIME
      COMMON /TO_KEEP_ORDER5/KEEP_ORDER5
      DATA FIRSTTIME / .TRUE. /
      INTEGER AMP_ORDERS(NSPLITORDERS)
      DOUBLE PRECISION ANS_MAX, TINY
      PARAMETER (TINY = 1D-12)
...
C     look for orders which match the nlo order constraint 
      ...
      IF (FIRSTTIME) THEN
        KEEP_ORDER5(:) = .TRUE.
        CALL FIRSTTIME5_SETUP(FIRSTTIME,KEEP_ORDER5)
      ENDIF
...
C     AMP_SPLIT(1:AMP_SPLIT_SIZE) = RET_AMP_SPLIT(1:AMP_SPLIT_SIZE)
      ...
      RECURSIVE SUBROUTINE SMATRIX5_AMP_VEC(P,
...
C     if (firsttime) then
C     call firsttime5_setup(firsttime,keep_order)
C     endif

      CALL SMATRIX5_SPLITORDERS_VEC(P,ANS,IVEC,FLAV_IDX)
      ANS_SUMMED = 0D0
      ANS_MAX = 0D0
...
C     avoid fake non-zeros
      ...
      SUBROUTINE FIRSTTIME5_SETUP(FIRSTTIME,KEEP_ORDER)
C     
...
C         identical matrix elements
          CALL MATRIX_5(P,NHEL(1,IHEL),FLAV_IDX,T)
          DO I=0,NSQAMPSO
...
C       add to the sum of helicities
     ...
     $        .NOT. GOODHEL(IHEL,HEL_INDEX)) THEN
      ...
      RECURSIVE SUBROUTINE SMATRIX5_SPLITORDERS_VEC(P,
     $     ANS,IVEC,FLAV_IDX)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C     SAVE T_SAVE,T_IDENT
      ...
      DATA (NHEL(I,  31),I=1,5) / 1, 1, 1,-1, 1/
      DATA (NHEL(I,  32),I=1,5) / 1, 1, 1,-1,-1/
      LOGICAL GOODHEL5(NCOMB), DO_TRY
      DATA DO_TRY/.TRUE./
      COMMON /TO_GOODHEL5/GOODHEL5
      INTEGER IDEN_FLAVOR(4)
      DATA IDEN_FLAVOR / 12, 12, 12, 12 /
C     ----------
C     BEGIN CODE
C     ----------
      ...
      IF (FLAV_IDX.GE.1.AND.FLAV_IDX.LE.4) IDEN=IDEN_FLAVOR(FLAV_IDX)
C     IF (DO_TRY) THEN
C     CALL SMATRIX5_SPLITORDERS_NTRY_VEC(P,ANS,DO_TRY,GOODHEL5,T_IDENT5
C     ,IVEC)
C     ELSE
      DO I=0,NSQAMPSO
        ANS(I) = 0D0
      ...
      DO IHEL=1,NCOMB
        T(:) = 0D0
        CALL MATRIX_5_VEC(P,NHEL(1,IHEL),FLAV_IDX,T,
     $       IVEC)
C       add to the sum of helicities
        DO I=1,NSQAMPSO  !keep loop from 1!!
          ANS(I)=ANS(I)+T(I)
...
C     ENDIF
      ...
      SUBROUTINE SMATRIX5_SPLITORDERS_NTRY(P,ANS,DO_TRY,GOODHEL
     $ ,T_IDENT)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C       add to the sum of helicities
      ...
      SUBROUTINE SMATRIX5_SPLITORDERS_NTRY_VEC(P,ANS,
...
C       add to the sum of helicities
      ...
      SUBROUTINE MATRIX_5(P,NHEL,FLAV_IDX,RES)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     JAMPs contributing to orders QCD=2 QED=1
     ...
     $       *DCONJG(JAMP(I,N))
      ...
      RECURSIVE SUBROUTINE MATRIX_5_VEC(P,NHEL,FLAV_IDX,
     $     RES,IVEC)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C     
      ...
      RECURSIVE INTEGER FUNCTION SQSOINDEX5(AMPORDERA,AMPORDERB)
C     
...
C     
     ...
     $   +AMPSPLITORDERS(AMPORDERB,I)
      ...
      RECURSIVE INTEGER FUNCTION SQSOINDEX_FROM_ORDERS5(ORDERS)
C     
...
C     
 ...
 1009   CONTINUE
     ...
     $ ,NSPLITORDERS)
      ...
      RECURSIVE INTEGER FUNCTION GETORDPOWFROMINDEX5(IORDER, INDX)
C     
...
C     
      ...
      RECURSIVE SUBROUTINE GET_NSQSO_REAL5(NSQSO)
C     
