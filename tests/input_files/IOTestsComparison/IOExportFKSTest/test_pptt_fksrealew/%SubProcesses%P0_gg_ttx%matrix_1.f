...
C     
C     
C     Process: _anti_quark g > t t~ _anti_quark [ real = QCD QED ]
C      QCD^2<=4 QED^2<=2
C     
C     
C     CONSTANTS
...
C     
      ...
      CALL SMATRIX1_AMP(P,DUMMY_AMP_SPLIT,ANS_SUMMED)
      ...
      SUBROUTINE SMATRIX1_AMP(P,RET_AMP_SPLIT,ANS_SUMMED)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     
      INTEGER I,J
      REAL*8 ANS(0:NSQAMPSO)
      LOGICAL KEEP_ORDER1(NSQAMPSO), FIRSTTIME
      COMMON /TO_KEEP_ORDER1/KEEP_ORDER1
      DATA FIRSTTIME / .TRUE. /
      INTEGER AMP_ORDERS(NSPLITORDERS)
      DOUBLE PRECISION ANS_MAX, TINY
      PARAMETER (TINY = 1D-12)
...
C     look for orders which match the nlo order constraint 
      ...
      IF (FIRSTTIME) THEN
        KEEP_ORDER1(:) = .TRUE.
        CALL FIRSTTIME1_SETUP(FIRSTTIME,KEEP_ORDER1)
      ENDIF
...
C     AMP_SPLIT(1:AMP_SPLIT_SIZE) = RET_AMP_SPLIT(1:AMP_SPLIT_SIZE)
      ...
      RECURSIVE SUBROUTINE SMATRIX1_AMP_VEC(P,
...
C     if (firsttime) then
C     call firsttime1_setup(firsttime,keep_order)
C     endif

      CALL SMATRIX1_SPLITORDERS_VEC(P,ANS,IVEC,FLAV_IDX)
      ANS_SUMMED = 0D0
      ANS_MAX = 0D0
...
C     avoid fake non-zeros
      ...
      SUBROUTINE FIRSTTIME1_SETUP(FIRSTTIME,KEEP_ORDER)
C     
...
C         identical matrix elements
          CALL MATRIX_1(P,NHEL(1,IHEL),FLAV_IDX,T)
          DO I=0,NSQAMPSO
...
C       add to the sum of helicities
     ...
     $        .NOT. GOODHEL(IHEL,HEL_INDEX)) THEN
      ...
      RECURSIVE SUBROUTINE SMATRIX1_SPLITORDERS_VEC(P,
     $     ANS,IVEC,FLAV_IDX)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C     SAVE T_SAVE,T_IDENT
      ...
      DATA (NHEL(I,  31),I=1,5) / 1, 1, 1,-1, 1/
      DATA (NHEL(I,  32),I=1,5) / 1, 1, 1,-1,-1/
      LOGICAL GOODHEL1(NCOMB), DO_TRY
      DATA DO_TRY/.TRUE./
      COMMON /TO_GOODHEL1/GOODHEL1
      INTEGER IDEN_FLAVOR(4)
      DATA IDEN_FLAVOR / 96, 96, 96, 96 /
C     ----------
C     BEGIN CODE
C     ----------
      ...
      IF (FLAV_IDX.GE.1.AND.FLAV_IDX.LE.4) IDEN=IDEN_FLAVOR(FLAV_IDX)
C     IF (DO_TRY) THEN
C     CALL SMATRIX1_SPLITORDERS_NTRY_VEC(P,ANS,DO_TRY,GOODHEL1,T_IDENT1
C     ,IVEC)
C     ELSE
      DO I=0,NSQAMPSO
        ANS(I) = 0D0
      ...
      DO IHEL=1,NCOMB
        T(:) = 0D0
        CALL MATRIX_1_VEC(P,NHEL(1,IHEL),FLAV_IDX,T,
     $       IVEC)
C       add to the sum of helicities
        DO I=1,NSQAMPSO  !keep loop from 1!!
          ANS(I)=ANS(I)+T(I)
...
C     ENDIF
      ...
      SUBROUTINE SMATRIX1_SPLITORDERS_NTRY(P,ANS,DO_TRY,GOODHEL
     $ ,T_IDENT)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C       add to the sum of helicities
      ...
      SUBROUTINE SMATRIX1_SPLITORDERS_NTRY_VEC(P,ANS,
...
C       add to the sum of helicities
      ...
      SUBROUTINE MATRIX_1(P,NHEL,FLAV_IDX,RES)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Flavor table for the FLAV_IDX -> FLAVOR rebuild.
      ...
      DATA FLAV_TABLE / 1, 1, 1, 1, 1, 2, 1, 1, 1, 2, 3, 1, 1, 1, 3, 4
     $ , 1, 1, 1, 4 /
C     
C     FUNCTION
C     
...
C     Amplitude(s) for diagram number 1
      CALL FFV1M_0(W(5),W(6),W(7),FLV_94,AMP(1))
      CALL FFV1P0_3(W(4),W(3),GC_11,ZERO,ZERO,W(8))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(5),W(6),W(8),GC_11,AMP(2))
      CALL FFV6_2_3(W(4),W(3),GC_FFV_4,GC_FFV_5,MDL_MZ,MDL_WZ,W(9))
C     Amplitude(s) for diagram number 3
      CALL FFV6_2M_0(W(5),W(6),W(9),FLV_96,FLV_97,AMP(3))
      CALL FFV1MP0_3(W(5),W(1),FLV_94,ZERO,ZERO,W(6))
...
C     Amplitude(s) for diagram number 5
      ...
      CALL FFV6_2M_3(W(5),W(1),FLV_96,FLV_97,MDL_MZ,MDL_WZ,W(12))
C     Amplitude(s) for diagram number 6
      CALL FFV6_2_0(W(4),W(10),W(12),GC_FFV_4,GC_FFV_5,AMP(6))
...
C     JAMPs contributing to orders QCD=1 QED=2
     ...
     $       *DCONJG(JAMP(I,N))
      ...
      RECURSIVE SUBROUTINE MATRIX_1_VEC(P,NHEL,FLAV_IDX,
     $     RES,IVEC)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C     Rebuild FLAVOR(NEXTERNAL) from the resolved flavor index.
      ...
      CALL IXXXXX(P(0,4),MDL_MT,NHEL(4),-1, FLAVOR(4),W(4))
      CALL IXXXXX(P(0,5),ZERO,NHEL(5),-1, FLAVOR(5),W(5))
      CALL FFV1_1(W(1),W(2),GC_11_VEC(IVEC),ZERO,ZERO,W(6))
      CALL FFV1P0_3(W(4),W(3),GC_2,ZERO,ZERO,W(7))
C     Amplitude(s) for diagram number 1
      CALL FFV1M_0(W(5),W(6),W(7),FLV_94,AMP(1))
      CALL FFV1P0_3(W(4),W(3),GC_11_VEC(IVEC),ZERO,ZERO,W(8))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(5),W(6),W(8),GC_11_VEC(IVEC),AMP(2))
      CALL FFV6_2_3(W(4),W(3),GC_FFV_4,GC_FFV_5,MDL_MZ,MDL_WZ,W(9))
C     Amplitude(s) for diagram number 3
      CALL FFV6_2M_0(W(5),W(6),W(9),FLV_96,FLV_97,AMP(3))
      CALL FFV1MP0_3(W(5),W(1),FLV_94,ZERO,ZERO,W(6))
      CALL FFV1_1(W(3),W(2),GC_11_VEC(IVEC),MDL_MT,MDL_WT,W(10))
C     Amplitude(s) for diagram number 4
      CALL FFV1_0(W(4),W(10),W(6),GC_2,AMP(4))
      CALL FFV1P0_3(W(5),W(1),GC_11_VEC(IVEC),ZERO,ZERO,W(11))
C     Amplitude(s) for diagram number 5
      CALL FFV1_0(W(4),W(10),W(11),GC_11_VEC(IVEC),AMP(5))
      CALL FFV6_2M_3(W(5),W(1),FLV_96,FLV_97,MDL_MZ,MDL_WZ,W(12))
C     Amplitude(s) for diagram number 6
      CALL FFV6_2_0(W(4),W(10),W(12),GC_FFV_4,GC_FFV_5,AMP(6))
      CALL FFV1_2(W(4),W(2),GC_11_VEC(IVEC),MDL_MT,MDL_WT,W(10))
C     Amplitude(s) for diagram number 7
      CALL FFV1_0(W(10),W(3),W(6),GC_2,AMP(7))
C     Amplitude(s) for diagram number 8
      CALL FFV1_0(W(10),W(3),W(11),GC_11_VEC(IVEC),AMP(8))
C     Amplitude(s) for diagram number 9
      CALL FFV6_2_0(W(10),W(3),W(12),GC_FFV_4,GC_FFV_5,AMP(9))
C     Amplitude(s) for diagram number 10
      CALL VVV1_0(W(11),W(2),W(8),GC_10_VEC(IVEC),AMP(10))
      CALL FFV1_2(W(5),W(2),GC_11_VEC(IVEC),ZERO,ZERO,W(11))
C     Amplitude(s) for diagram number 11
      CALL FFV1M_0(W(11),W(1),W(7),FLV_94,AMP(11))
C     Amplitude(s) for diagram number 12
      CALL FFV1_0(W(11),W(1),W(8),GC_11_VEC(IVEC),AMP(12))
C     Amplitude(s) for diagram number 13
      CALL FFV6_2M_0(W(11),W(1),W(9),FLV_96,FLV_97,AMP(13))
C     JAMPs contributing to orders QCD=3 QED=0
      JAMP(1,1) = (5.000000000000000D-01)*AMP(5)+((0.000000000000000D
...
C     
      ...
      RECURSIVE INTEGER FUNCTION SQSOINDEX1(AMPORDERA,AMPORDERB)
C     
...
C     
     ...
     $   +AMPSPLITORDERS(AMPORDERB,I)
      ...
      RECURSIVE INTEGER FUNCTION SQSOINDEX_FROM_ORDERS1(ORDERS)
C     
...
C     
 ...
 1009   CONTINUE
     ...
     $ ,NSPLITORDERS)
      ...
      RECURSIVE INTEGER FUNCTION GETORDPOWFROMINDEX1(IORDER, INDX)
C     
...
C     
      ...
      RECURSIVE SUBROUTINE GET_NSQSO_REAL1(NSQSO)
C     
