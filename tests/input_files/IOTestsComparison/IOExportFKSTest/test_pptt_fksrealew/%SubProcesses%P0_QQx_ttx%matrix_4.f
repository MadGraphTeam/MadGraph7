...
C     
      ...
      CALL SMATRIX4_AMP(P,DUMMY_AMP_SPLIT,ANS_SUMMED)
      ...
      SUBROUTINE SMATRIX4_AMP(P,RET_AMP_SPLIT,ANS_SUMMED)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     
      INTEGER I,J
      REAL*8 ANS(0:NSQAMPSO)
      LOGICAL KEEP_ORDER4(NSQAMPSO), FIRSTTIME
      COMMON /TO_KEEP_ORDER4/KEEP_ORDER4
      DATA FIRSTTIME / .TRUE. /
      INTEGER AMP_ORDERS(NSPLITORDERS)
      DOUBLE PRECISION ANS_MAX, TINY
      PARAMETER (TINY = 1D-12)
...
C     look for orders which match the nlo order constraint 
      ...
      IF (FIRSTTIME) THEN
        KEEP_ORDER4(:) = .TRUE.
        CALL FIRSTTIME4_SETUP(FIRSTTIME,KEEP_ORDER4)
      ENDIF
...
C     AMP_SPLIT(1:AMP_SPLIT_SIZE) = RET_AMP_SPLIT(1:AMP_SPLIT_SIZE)
      ...
      RECURSIVE SUBROUTINE SMATRIX4_AMP_VEC(P,
...
C     if (firsttime) then
C     call firsttime4_setup(firsttime,keep_order)
C     endif

      CALL SMATRIX4_SPLITORDERS_VEC(P,ANS,IVEC,FLAV_IDX)
      ANS_SUMMED = 0D0
      ANS_MAX = 0D0
...
C     avoid fake non-zeros
      ...
      SUBROUTINE FIRSTTIME4_SETUP(FIRSTTIME,KEEP_ORDER)
C     
...
C         identical matrix elements
          CALL MATRIX_4(P,NHEL(1,IHEL),FLAV_IDX,T)
          DO I=0,NSQAMPSO
...
C       add to the sum of helicities
     ...
     $        .NOT. GOODHEL(IHEL,HEL_INDEX)) THEN
      ...
      RECURSIVE SUBROUTINE SMATRIX4_SPLITORDERS_VEC(P,
     $     ANS,IVEC,FLAV_IDX)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C     SAVE T_SAVE,T_IDENT
      ...
      DATA (NHEL(I,  31),I=1,5) / 1, 1, 1,-1, 1/
      DATA (NHEL(I,  32),I=1,5) / 1, 1, 1,-1,-1/
      LOGICAL GOODHEL4(NCOMB), DO_TRY
      DATA DO_TRY/.TRUE./
      COMMON /TO_GOODHEL4/GOODHEL4
      INTEGER IDEN_FLAVOR(4)
      DATA IDEN_FLAVOR / 96, 96, 96, 96 /
C     ----------
C     BEGIN CODE
C     ----------
      ...
      IF (FLAV_IDX.GE.1.AND.FLAV_IDX.LE.4) IDEN=IDEN_FLAVOR(FLAV_IDX)
C     IF (DO_TRY) THEN
C     CALL SMATRIX4_SPLITORDERS_NTRY_VEC(P,ANS,DO_TRY,GOODHEL4,T_IDENT4
C     ,IVEC)
C     ELSE
      DO I=0,NSQAMPSO
        ANS(I) = 0D0
      ...
      DO IHEL=1,NCOMB
        T(:) = 0D0
        CALL MATRIX_4_VEC(P,NHEL(1,IHEL),FLAV_IDX,T,
     $       IVEC)
C       add to the sum of helicities
        DO I=1,NSQAMPSO  !keep loop from 1!!
          ANS(I)=ANS(I)+T(I)
...
C     ENDIF
      ...
      SUBROUTINE SMATRIX4_SPLITORDERS_NTRY(P,ANS,DO_TRY,GOODHEL
     $ ,T_IDENT)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C       add to the sum of helicities
      ...
      SUBROUTINE SMATRIX4_SPLITORDERS_NTRY_VEC(P,ANS,
...
C       add to the sum of helicities
      ...
      SUBROUTINE MATRIX_4(P,NHEL,FLAV_IDX,RES)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     JAMPs contributing to orders QCD=1 QED=2
     ...
     $       *DCONJG(JAMP(I,N))
      ...
      RECURSIVE SUBROUTINE MATRIX_4_VEC(P,NHEL,FLAV_IDX,
     $     RES,IVEC)
C     
C     Generated by MadGraph7 v. %(version)s, %(date)s
C     By the MadGraph7 Development Team
...
C     
      ...
      RECURSIVE INTEGER FUNCTION SQSOINDEX4(AMPORDERA,AMPORDERB)
C     
...
C     
     ...
     $   +AMPSPLITORDERS(AMPORDERB,I)
      ...
      RECURSIVE INTEGER FUNCTION SQSOINDEX_FROM_ORDERS4(ORDERS)
C     
...
C     
 ...
 1009   CONTINUE
     ...
     $ ,NSPLITORDERS)
      ...
      RECURSIVE INTEGER FUNCTION GETORDPOWFROMINDEX4(IORDER, INDX)
C     
...
C     
      ...
      RECURSIVE SUBROUTINE GET_NSQSO_REAL4(NSQSO)
C     
