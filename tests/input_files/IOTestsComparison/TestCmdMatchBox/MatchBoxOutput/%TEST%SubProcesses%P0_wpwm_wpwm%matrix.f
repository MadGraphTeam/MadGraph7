      SUBROUTINE MG5_0_SMATRIX(P, FLAV_IDX, ANS)
C     
...
C      the
C     requested flavor -- not an allowed combination, matrix element
C      is zero.
      IF (FLAV_IDX.LT.1 .OR. FLAV_IDX.GT.NFLAV) THEN
        ANS = 0D0
        RETURN
      ENDIF
      CALL MG5_0_GET_FLAVOR(FLAV_IDX, FLAVOR)
      NTRY(FLAV_IDX)=NTRY(FLAV_IDX)+1
      ANS = 0D0
      DO IHEL=1,NCOMB
        IF (GOODHEL(IHEL,FLAV_IDX) .OR. NTRY(FLAV_IDX) .LT. 20) THEN
          T=MG5_0_MATRIX(P ,NHEL(1,IHEL),FLAV_IDX)
          ANS=ANS+T
      ...
      REAL*8 FUNCTION MG5_0_MATRIX(P,NHEL,FLAV_IDX)
      USE MODEL_OBJECT
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
...
C     BEGIN CODE
C     ----------
      FLAV_IDX = MG5_0_GET_FLAVOR_INDEX(FLAVOR)
...
C     Amplitude(s) for diagram number 7
     ...
     $             +K-1)) THEN
      ...
      INTEGER FUNCTION MG5_0_GET_FLAVOR_INDEX(FLAVOR)
...
C     0 sentinel for an unresolved (not-in-table) flavor (see above).
      ...
      SUBROUTINE MG5_0_GET_FLAVOR(FLAV_IDX, FLAVOR)
