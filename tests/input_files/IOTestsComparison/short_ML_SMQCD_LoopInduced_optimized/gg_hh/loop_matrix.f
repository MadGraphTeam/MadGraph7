...
C       SKIP THE ONES THAT NOT AVAILABLE
     ...
     $     //' MLReductionLib is correct.'
        ENDIF
C       The poles must vanish here, but COLLIER can be asked not to
C        compute them at all.
        IF (MLPOLECHECKTHRES.GT.0.0D0.AND.MLREDUCTIONLIB(1)
     $   .EQ.7.AND.(.NOT.COLLIERCOMPUTEUVPOLES.OR..NOT.COLLIERCOMPUTEIR
     $POLES)) THEN
          WRITE(*,*) '##INFO: The vanishing-pole check of this loop'
     $     //'-induced process is inactive because COLLIER is not'
     $     //' computing the poles. A madevent run turns them off on'
     $     //' purpose and the card cannot override that; in a'
     $     //' standalone run, set COLLIERComputeUVpoles and'
     $     //' COLLIERComputeIRpoles to .TRUE. in MadLoopParams.dat.'
        ENDIF
        J=0
        DO I=1,NLOOPLIB
...
C     This code is therefore placed before 205 and after 200.
      CALL ML5_0_REINITIALIZE_CUMULATIVE_ARRAYS()
...
C      subroutine ML5_0_MP_COMPUTE_LOOP_COEFS to compute OpenLoop
...
C     Nothing to compare against if the reduction tool does not
C      compute them.
      POLES_COMPUTED = .NOT.(MLREDUCTIONLIB(I_LIB)
     $ .EQ.7.AND.(.NOT.COLLIERCOMPUTEUVPOLES.OR..NOT.COLLIERCOMPUTEIRPO
     $LES))
      DO_POLE_CHECK =
     $  MLPOLECHECKTHRES.GT.0.0D0.AND.POLES_COMPUTED.AND.NTRY.GT.0
      DO_POLE_CHECK =
     $  DO_POLE_CHECK.AND..NOT.CHECKPHASE.AND.HELDOUBLECHECKED
      DO_POLE_CHECK = DO_POLE_CHECK.AND.RET_CODE_H.NE.4.AND.ANS(1,0)
     $ .NE.0.0D0
...
C     arrays
      ...
      CALL ML5_0_DEALLOCATE_COLOR_FLOWS()
      CONTINUE
      END
