...
C     
      ...
      DOUBLE PRECISION B1_COMPONENTS(N_EE)
      DOUBLE PRECISION BX2_COMPONENTS(N_EE)
...
C         
      ...
      IF (ABS(LPP(1)) .GE. 1) THEN
        B1=PDG2PDF(LPP(1),5,1,XBK(1),DSQRT(Q2FACT(1)))
        IF ((ABS(LPP(1)).EQ.4.OR.ABS(LPP(1)).EQ.3)
     $   .AND.PDLABEL.NE.'none') B1_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
      ENDIF
      IF (ABS(LPP(2)) .GE. 1) THEN
        BX2=PDG2PDF(LPP(2),-5,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') BX2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! b b~ > z z
      PD(IPROC) = B1*BX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(B1_COMPONENTS,BX2_COMPONENTS)
      DO I=1,IPROC
