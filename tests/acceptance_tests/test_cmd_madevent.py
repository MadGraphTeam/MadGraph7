...
set draw_rivet_plots True
    ...
    def test_w_production_with_ms_decay_mg7(self):
        ...
        EXPECTED FAILURE for now: the command interface drives the chain
        correctly (generation runs and MadSpin is invoked), but MadSpin aborts
        with "Invalid proc_card information in the file (no generate line)" --
        the mg7 LHE banner does not yet carry the proc_card in the form MadSpin's
        do_import expects, so no run_01_decayed_* events are produced. Kept as an
        @expectedFailure (mirroring test_madspin_mixed_flavor_decay_log_summary_
        mg7) so the gap stays tracked; it flips to an unexpected success once
        mg7 + MadSpin works, which is the signal to drop this decorator."""
        import glob
        datadir = _mg7_datadir_or_skip(self)
