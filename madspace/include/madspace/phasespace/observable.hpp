...
public:
    ...
    /// Whether @p observable is unchanged by the initial-state mirror, the
    /// rotation by pi about x (py, pz -> -py, -pz) that moves each leg onto
    /// the other beam.
    static bool mirror_invariant(ObservableOption observable);
    /// Whether this observable is mirror invariant. Both the observable itself
    /// and the one it is ordered by have to be: sorting by a quantity that
    /// flips picks a different particle out of the event.
