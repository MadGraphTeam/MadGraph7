...
public:
    ...
    static constexpr int f_channel_info = 128; // SDE channel + selected diagram
    ...
    static std::vector<FieldLayout> layout(int fields) {
        ...
        if (fields & f_channel_info) {
            ret.insert(
                ret.end(),
                {
                    {"sde_channel", FieldLayout::i32, 7},
                    {"selected_channel", FieldLayout::i32, 7},
                }
            );
        }
        return ret;
    ...
    // SDE channel info: group representative + per-event sampled diagram
    UnalignedRef<int> sde_channel() { return &_data[_offsets[6] + 0]; }
    UnalignedRef<int> selected_channel() { return &_data[_offsets[6] + 4]; }
