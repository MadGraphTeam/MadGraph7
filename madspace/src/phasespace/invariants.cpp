...
Mapping::Result Invariant::build_forward_impl(
    ...
    Value s, virt, det;
    if (_width != 0) {
        auto result = fb.breit_wigner_invariant(r, _mass, _width, s_min, s_max);
        s = result[0];
        virt = result[1];
        det = result[2];
    } else if (_power == 0) {
        auto result = fb.uniform_invariant(r, s_min, s_max);
        s = result[0];
        det = result[1];
        virt = s;
    } else if (_power == 1) {
        auto result = fb.stable_invariant(r, _mass, s_min, s_max);
        s = result[0];
        virt = result[1];
        det = result[2];
    } else {
        auto result = fb.stable_invariant_nu(r, _mass, _power, s_min, s_max);
        s = result[0];
        virt = result[1];
        det = result[2];
    }
...
Mapping::Result Invariant::build_inverse_impl(
    ...
    Value r, det;
    if (_width != 0) {
        auto result = fb.breit_wigner_invariant_inverse(s, _mass, _width, s_min, s_max);
        r = result[0];
        det = result[1];
    } else if (_power == 0) {
        auto result = fb.uniform_invariant_inverse(s, s_min, s_max);
        r = result[0];
        det = result[1];
    } else if (_power == 1) {
        auto result = fb.stable_invariant_inverse(s, _mass, s_min, s_max);
        r = result[0];
        det = result[1];
    } else {
        auto result = fb.stable_invariant_nu_inverse(s, _mass, _power, s_min, s_max);
        r = result[0];
        det = result[1];
    }
    return {{{"random", r}}, det};
