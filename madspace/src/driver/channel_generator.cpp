...
ChannelEventGenerator::ChannelEventGenerator(
    ...
    _event_layout_extra_flags(
        event_extra_flags(integrand.return_types().index_map()) |
        EventRecord::f_channel_info
    ),
...
void ChannelEventGenerator::init_field_indices() {
    ...
    _field_indices.channel_index = index_map.at("channel_index");
    if (index_map.contains("fact_scale1")) {
...
void ChannelEventGenerator::write_events(
    ...
    auto channel_view =
        unweighted_events.at(_field_indices.channel_index).view<me_int_t, 1>();
    ...
    for (std::size_t i = 0; i < w_view.size(); ++i) {
        ...
        event.sde_channel() = _status.sde_channel;
        // map the sampled internal channel-weight index to its diagram number:
        // the channel's _channel_indices is the contiguous range
        // [channel_base, channel_base + diagram_group.size())
        int selected = channel_view[i];
